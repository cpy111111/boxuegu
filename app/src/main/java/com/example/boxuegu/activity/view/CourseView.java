...
import com.example.boxuegu.activity.adapter.AdBannerAdapter;
import com.example.boxuegu.activity.adapter.CourseAdapter;
import com.example.boxuegu.activity.bean.BannerBean;
import com.example.boxuegu.activity.bean.CourseBean;
import com.example.boxuegu.activity.utils.Constant;
import com.example.boxuegu.activity.utils.JsonParse;
...
public class CourseView {
    ...
    private void initView() {
        mHandler = new MHandler();
         //System.out.println("aaaaaaaaaaaaaaaaaaaa111111");
    ...
    class AdAutoSlidThread extends Thread { // 实现广告栏每隔5秒自动滑动的功能
        @Override
        public void run() {
            super.run();
            while (true) {
            ...
            switch (msg.what) {
                case MSG_COURSE_OK:
                    if (msg.obj != null) {
                        ...
                        List<CourseBean> cbl = JsonParse.getInstance().getCourseList(vlResult);
                ...
                case MSG_BANNER_OK:
                    if (msg.obj != null) {
                        String vlResult = (String) msg.obj;
                        List<BannerBean> bbl = JsonParse.getInstance().getBannerList(vlResult);
                    ...
                    break;
                case MSG_AD_SLID:
                    if (ada.getCount() > 0) { // 设置滑动到下一张广告图片
    ...
    private void resetSize() { // 设置广告栏布局的宽度与高度
